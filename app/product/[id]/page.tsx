...
export default function ProductDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isLoading) {
    return (
      <>
        <Header />
        <main className="flex-grow bg-white py-10 flex items-center justify-center min-h-[60vh]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="flex flex-col gap-4">
                <div className="relative w-full aspect-square rounded-2xl overflow-hidden flex items-center justify-center">
                  ...
                  <div className="w-full h-full bg-contain bg-center bg-no-repeat mix-blend-multiply" style={{ backgroundImage: `url('${ImageSizes.full(product.images[activeImageIndex])}')` }}></div>
                </div>
                <div className="grid grid-cols-4 gap-4">
                  {product.images.map((img: string, idx: number) => (
                    <button
                      ...
                      className={`relative aspect-square bg-slate-100 rounded-xl overflow-hidden border-2 ${idx === activeImageIndex ? 'border-primary' : 'border-transparent'} hover:border-primary/50 transition-colors p-2`}
                    >
                      <div className="w-full h-full bg-contain bg-center bg-no-repeat mix-blend-multiply" style={{ backgroundImage: `url('${ImageSizes.thumbnail(img)}')` }}></div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col">
                ...
                {availableColors.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wider">{t.product.color}: <span className="text-slate-500 font-normal normal-case">{selectedColor}</span></h3>
                ...
                {availableSizes.length > 0 && selectedColor && (
                  <div className="mb-8 border-b border-slate-100 pb-8">
                    <h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wider">{t.product.size}: <span className="text-slate-500 font-normal normal-case">{selectedSize}</span></h3>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={`px-5 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${selectedSize === v.size
                            ...
                            : v.stock === 0
                              ...
                              : 'border-slate-200 text-slate-600 hover:border-slate-300 hover:'
                            }`}
                        >
                ...
                <form ref={formRef} onSubmit={handleOrderSubmit} className="mb-4 flex flex-col gap-4 p-6 rounded-2xl">

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">{t.product.orderForm?.fullName || 'Nom complet'}</label>
                      <input
                        type="text"
                        required
                        value={orderName}
                        onChange={(e) => setOrderName(e.target.value)}
                        className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">{t.product.orderForm?.phone || 'Téléphone'}</label>
                      <input
                        type="tel"
                        required
                        pattern="^0[678][0-9]{8}$"
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">{t.product.orderForm?.address || 'Adresse'}</label>
                    <input
                      type="text"
                      required
                      value={orderAddress}
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">{t.product.orderForm?.city || 'Ville'}</label>
                    <input
                      type="text"
                      required
                      value={orderCity}
              ...
              <div className=" p-6 rounded-2xl relative overflow-hidden">
                {isReviewSubmitted ? (
                  <div className="flex flex-col items-center justify-center text-center py-12 animate-in fade-in zoom-in duration-500">
                    ...
                    <p className="text-slate-600 max-w-[300px] mb-8">{t.reviewsSection.successMessage || "Votre avis a été soumis avec succès et est en attente d'approbation par notre équipe."}</p>
                    <button
                    ...
                    <form onSubmit={handleSubmitReview} className="flex flex-col gap-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="flex flex-col gap-2">
                          ...
                          <input required type="text" id="reviewName" value={name} onChange={e => setName(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all" />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label htmlFor="reviewEmail" className="text-sm font-bold text-slate-700">{t.reviewsSection.email}</label>
                          <input required type="email" id="reviewEmail" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all" />
                        </div>
                      </div>

                      <div className="flex flex-col gap-2">
                        <label htmlFor="reviewComment" className="text-sm font-bold text-slate-700">{t.reviewsSection.comment}</label>
                        <textarea required id="reviewComment" rows={4} value={comment} onChange={e => setComment(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all resize-none"></textarea>
                      </div>

                      <div className="flex flex-col gap-2">
                        <label htmlFor="reviewImage" className="text-sm font-bold text-slate-700">{t.reviewsSection.image}</label>
                        <input type="file" id="reviewImage" accept="image/*" onChange={handleImageChange} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" />
                      </div>
      ...
      >
        <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-4">
          <div className="hidden sm:flex items-center gap-4 flex-1">
            <div
              className="w-12 h-12 bg-slate-100 rounded-lg bg-cover bg-center"
